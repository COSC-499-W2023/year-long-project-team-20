...
import { getRequestVideo } from "../graphql/queries";
import { updateRequestVideo } from "../graphql/mutations";
export default function RequestVideoUpdateForm(props) {
  const {
    created: createdProp,
    requestVideo: requestVideoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = requestVideoRecord
      ? { ...initialValues, ...requestVideoRecord }
      : initialValues;
    setCreated(cleanValues.created);
    setDuedate(cleanValues.duedate);
    setFrom(cleanValues.from);
    setTo(cleanValues.to);
    setFromEmail(cleanValues.fromEmail);
    setToEmail(cleanValues.toEmail);
    setIsRead(cleanValues.isRead);
    setIsCompleted(cleanValues.isCompleted);
    setMessage(cleanValues.message);
    setErrors({});
  };
  const [requestVideoRecord, setRequestVideoRecord] = React.useState(
    requestVideoModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = createdProp
        ? (
            await API.graphql({
              query: getRequestVideo.replaceAll("__typename", ""),
              variables: { created: createdProp },
            })
          )?.data?.getRequestVideo
        : requestVideoModelProp;
      setRequestVideoRecord(record);
    };
    queryData();
  }, [createdProp, requestVideoModelProp]);
  React.useEffect(resetStateValues, [requestVideoRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateRequestVideo.replaceAll("__typename", ""),
            variables: {
              input: {
                created: requestVideoRecord.created,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "RequestVideoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Created"
      ...
      >
        <Button
          ...
          isDisabled={!(createdProp || requestVideoModelProp)}

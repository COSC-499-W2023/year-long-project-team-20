...
import { listVideoLists } from "../graphql/queries";
...
const nextToken = {};
const apiCache = {};
export default function VideoCardCollection(props) {
  ...
  const loadPage = async (page) => {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await API.graphql({
          query: listVideoLists.replaceAll("__typename", ""),
          variables,
        })
      ).data.listVideoLists;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "VideoCardCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
